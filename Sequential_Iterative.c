...
int main()
{
    ...
    for(int i=0; i<num_samples; i+=chunk_size){
        ...
        for(int j=0; j<chunk_size; j++){
            chunk_real[j] = buffer_real[j + i];
            chunk_imag[j] = buffer_imag[j + i];
        }

        fft(chunk_real, chunk_imag, chunk_size);

        chunk_real[1] = chunk_real[1] * 1;
        

        ifft(chunk_real, chunk_imag, chunk_size);

        
      
        for(int j=0; j<chunk_size; j++){
            buffer_real[j+i] = chunk_real[j];
            buffer_imag[j+i] = chunk_imag[j];
        }
    }
